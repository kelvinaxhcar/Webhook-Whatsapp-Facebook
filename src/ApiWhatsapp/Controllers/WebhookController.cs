...
public class WebhookController : ControllerBase {
  ...
  private async Task<string> ObterMensagem(Root res) {
    if (res.entry.First().changes.First().value.messages != null) {
      if (res.entry.First().changes.First().value.messages.First().type ==
          "text") {
        return res.entry.First()
            .changes.First()
  ...
  //    openAiService.Completions.CreateCompletion(new CompletionCreateRequest()
  //    {
  //        Prompt = mensagem,
  ...
  //        mensagemDeRetorno = $"Erro {completionResult.Error.Code}:
